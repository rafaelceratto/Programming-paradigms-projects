...
object TextProcessing {
  def formatDateFromUTC(utc: Long): String = {
    Instant.ofEpochSecond(utc).toString
  }
  ...
  def countWords(text : String): Map[String, Int] ={
      ...
      val textFilter = textList.filter{ txt =>
        txt.nonEmpty && //chequeo esto por si split falla
        txt.head.isUpper &&
        !stopwords.contains(txt)
      }
      textFilter.groupBy{word => word}.mapValues{list => list.length}.toMap
  }
}